...
/*global */
/*eslint no-unused-vars: ["error", { "varsIgnorePattern": ".+", argsIgnorePattern:".+" }]*/
...
(function($){
    ...
    $.fn.destroyAreacomplete = function(id){
        for (let i of Object.keys(_data)) {
            if (_data[i].ta.id == id){
                _data[i].clone.remove()
                _data[i].list.remove()
            }
        }
    }
    ...
    function getWords(data){
        ...
        while( wordsFound < data.wordCount && pos >= 0 && text.charAt(pos) != '\n'){
            ...
            pos--
            if( text.charAt(pos) == ' ' || text.charAt(pos) == '(' || pos < 0 ){
                wordsFound++
            }
        }
        return ret.join('')
    ...
    function getCursorPosition(data){
        ...
        // console.log('ta-clone-left', $(ta).offset().left - miror.offset().left)
        // console.log('ta-clone-top', $(ta).offset().top - miror.offset().top)

        return {
            top:offset.top+span.height()+$(ta).offset().top - miror.offset().top, 
            left:offset.left+span.width()+$(ta).offset().left - miror.offset().left
        }
